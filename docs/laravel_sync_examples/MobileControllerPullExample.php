...
class MobileController extends Controller
{
    public function pull(Request $request): JsonResponse
    {
        ...
        if ($request->scope === 'full') {
                ...
                $response['anares']['fronts'] = $this->getFronts($projectIds, $nowIso);
                $response['anares']['phases'] = $this->getPhases($projectIds, $nowIso);
                ...
                $response['actreu']['participants'] = $this->getParticipants($projectIds, $nowIso);
        ...
        $response['comments'] = $this->getComments($projectIds, $nowIso);
    ...
    private function getRestrictionModules(array $projectIds, string $nowIso)
    {
        return DB::table('anares_analisisrestricciones')
            ...
            ->select(['codAnaRes', 'codProyecto', 'codEstado', 'dayFechaCreacion', 'desUsuarioCreacion'])
            ->get()
    ...
    private function getMeetings(array $projectIds, string $nowIso)
    {
        return DB::table('actreu_reuniones as reu')
            ...
            ->select([
                ...
                DB::raw("
                    CASE
                        WHEN reu.codEstado = 1 THEN 'Pendiente'
    ...
    private function getAgreements(array $projectIds, Carbon $now)
    {
        ...
        return DB::table('actreu_acuerdos as acu')
            ...
            ->select([
                ...
                DB::raw("
                    CASE
                        WHEN acu.codEstado = 1 THEN 'Pendiente'
                        WHEN acu.codEstado = 2 THEN 'Aplazado'
                        WHEN acu.codEstado = 3 THEN 'Finalizado'
                        WHEN acu.codEstado IN (4,5) THEN 'Atraso'
            ...
            ->map(function ($row) use ($today, $nowIso) {
                ...
                if (in_array((int) $row->codEstado, [1, 2, 4, 5], true)) {
                    $row->is_pending = 1;
                }
    ...
    private function getComments(array $projectIds, string $nowIso)
    {
        return DB::table('actreu_comentarios_acuerdo as com')
            ->leftJoin('actreu_acuerdos as acu', 'com.codActReuAcuerdos', '=', 'acu.codActReuAcuerdos')
            ->leftJoin('actreu_reuniones as reu', 'acu.codActReuReuniones', '=', 'reu.codActReuReuniones')
            ->leftJoin('actreu_subcategoria as sub', 'reu.codActReuSubCategoria', '=', 'sub.codActReuSubCategoria')
            ->whereIn('sub.codProyecto', $projectIds)
            ->select([
                'com.codComentario',
                'acu.codActReuAcuerdos',
                ...
                DB::raw('com.dayFechaModificacion as updated_at'),
            ])
            ->get()
            ->map(function ($row) use ($nowIso) {
    ...
    {
        return DB::table('conhit_general')
            ->select([
                'codConHitGeneral',
                'codConHit',
                'codProyecto',
    ...
    {
        return DB::table('conhit_detallehitos')
            ->select([
                'codConHitDetalleHitos',
                'codConHit',
                'codProyecto',
                'codConHitGeneral',
                'NumOrden',
