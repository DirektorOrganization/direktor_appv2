...
class AppRepository {
  ...
  Future<AppBootstrapData> bootstrap() async {
    ...
    final currentProjectId = await _loadCurrentProjectId(db) ?? (projects.isEmpty ? null : projects.first.id);
  ...
  Future<AppBootstrapData> updateRestrictionStatus({required int restrictionId, required String statusCode}) async {
    final db = await _database.database;
    final maps = _statusFlags(statusCode);
    ...
    await db.update(
      'anares_restriction',
      {
        ...
        'sync_status': 'pending',
        'dayFechaModificacion': DateTime.now().toIso8601String(),
        ...maps,
      },
      where: 'codAnaResActividad = ?',
      whereArgs: [restrictionId],
  ...
  Future<AppBootstrapData> saveRestriction(RestrictionDraft draft) async {
    ...
    if (draft.id == null) {
      final nextId = (Sqflite.firstIntValue(
                await db.rawQuery('SELECT MAX(codAnaResActividad) FROM anares_restriction'),
              ) ??
              400) +
          1;
      ...
      await db.insert('sync_queue', {
        ...
        'payload_json': '{"codAnaResActividad":$nextId}',
        'status': 'pending',
        'created_at': now,
        'updated_at': now,
      });
      ...
      await db.insert('sync_queue', {
        ...
        'payload_json': '{"codAnaResActividad":${draft.id}}',
        'status': 'pending',
        'created_at': now,
        'updated_at': now,
      });
  ...
  Future<RestrictionRecord?> getRestrictionById(int id) async {
    final db = await _database.database;
  ...
  Future<ProjectSnapshot> _loadProjectSnapshot(Database db, int projectId) async {
    ...
    final meetingSummaryRows = await db.query(
      'meetings_summary',
      where: 'codProyecto = ?',
      whereArgs: [projectId],
      limit: 1,
    ...
    final meetingRows = await db.query(
      'meetings_meeting',
      where: 'codProyecto = ?',
      whereArgs: [projectId],
    ...
    final agreementRows = await db.query(
      ...
      where: 'codProyecto = ?',
      whereArgs: [projectId],
      orderBy: 'is_overdue DESC, is_pending DESC, codActReuAcuerdos ASC',
    );
  ...
  Future<RestrictionCatalogs> _loadCatalogs(Database db, int projectId) async {
    ...
    return RestrictionCatalogs(
      ...
      statuses: statuses.map((row) => CatalogOption(id: (row['codEstado'] as String?) ?? '', label: (row['desEstado'] as String?) ?? '', colorHex: row['iconColor'] as String?)).toList(),
    );
  }
  ...
  RestrictionRecord _mapRestriction(Map<String, Object?> row) {
    return RestrictionRecord(
      ...
      isCompleted: (row['is_completed'] as int? ?? 0) == 1,
      isOverdue: (row['is_overdue'] as int? ?? 0) == 1,
      isDueToday: (row['is_due_today'] as int? ?? 0) == 1,
      isPending: (row['is_pending'] as int? ?? 0) == 1,
      isInProgress: (row['is_in_progress'] as int? ?? 0) == 1,
      priorityOrder: (row['priority_order'] as int?) ?? 999,
